...
suite('Group By Folder', () => {
	vscode.window.showInformationMessage('Start tests for Folder.');
	...
	test('task node when completed=false, group=folder, prefix=ignore', () => {
		...
		assert.strictEqual(
			testData.validCase.groupFolder,
	...
	test('task node when completed=false, group=folder, prefix=override', () => {
		let config = {
			completed: false,
			groupBy: getTasks.groupByFolder,
	...
	test('task node when completed=false, group=folder, prefix=sub', () => {
		let config = {
			completed: false,
			groupBy: getTasks.groupByFolder,
		...
		assert.strictEqual(
			testData.validCase.groupFolderSub,
			getTasks.getTaskNode(testData.validCase.path, testData.validCase.taskOngoingWithPreview.match(getTasks.patternTask), config).group);
	...
	// completed=true
	test('task node when completed=true, group=folder, prefix=ignore', () => {
		let config = {
			completed: true,
			groupBy: getTasks.groupByFolder,
			prefix: getTasks.prefixIgnore
	...
	test('task node when completed=true, group=folder, prefix=override', () => {
		let config = {
			completed: true,
			groupBy: getTasks.groupByFolder,
			prefix: getTasks.prefixOverride
